...
(embedded into the full bus vector with the reference entries held at zero) which makes the flow sensitivity fully explicit:
...
where ``A_r`` is the incidence matrix with its reference bus columns removed (so ``A \,\partial\theta/\partial \mathrm{sw}_e = A_r \,\partial\theta_r/\partial \mathrm{sw}_e``), and we used ``a_{e,r}^\top \theta_r = a_e^\top \theta`` since ``\theta_{\text{refs}} = 0``. The shared scalar ``b_e \,(a_e^\top \theta) = b_e \,(A\theta)_e`` is exactly the negative of branch ``e``'s flow when it is in service (``f_e = -b_e (A\theta)_e`` at ``\mathrm{sw}_e = 1``), so the whole flow sensitivity scales with how heavily branch ``e`` is loaded.
