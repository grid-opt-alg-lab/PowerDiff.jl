...
# =============================================================================
# psh Finite-Difference Verification
#
# Uses a shedding-active 2-bus setup (gmax=0.5, d=[0.0, 1.0]) so that psh > 0
# and sensitivities are non-trivial.
# =============================================================================
@testset "psh Finite-Difference Verification" begin
    n, m, k = 2, 1, 1
    A = sparse([1.0 -1.0])
    G_inc = sparse(reshape([1.0, 0.0], 2, 1))
    b_base = [-10.0]
    ...
    delta = 1e-5
    psh_tol = 0.05  # 5% relative tolerance

    function make_psh_net(; b=b_base, fmax=[100.0], cl=[10.0], cq=[1.0])
        DCNetwork(n, m, k, A, G_inc, b;
            fmax=fmax, gmax=[0.5], gmin=[0.0],
            cl=cl, cq=cq, c_shed=[1e4, 1e4],
            ref_bus=1, τ=0.01)
    end

    function solve_psh_opf(net, demand=d)
        prob = DCOPFProblem(net, demand)
        sol = solve!(prob)
        return prob, sol
    end

    function check_psh(analytical_matrix, sol_base, sol_pert, col_idx)
        fd = (sol_pert.psh - sol_base.psh) / delta
        analytical_col = Matrix(analytical_matrix)[:, col_idx]
        if norm(fd) > 1e-10
            rel_err = norm(analytical_col - fd) / norm(fd)
            @test rel_err < psh_tol
        else
            @test norm(analytical_col) < 1e-6
        end
    end
    ...
    @testset "w.r.t. :cq" begin
        net_base = make_psh_net()
        prob_base, sol_base = solve_psh_opf(net_base)

        cq_pert = [1.0 + delta]
        net_pert = make_psh_net(cq=cq_pert)
        _, sol_pert = solve_psh_opf(net_pert)

        dpsh_dcq = calc_sensitivity(prob_base, :psh, :cq)
        check_psh(dpsh_dcq, sol_base, sol_pert, 1)
    end

    @testset "w.r.t. :cl" begin
        net_base = make_psh_net()
        prob_base, sol_base = solve_psh_opf(net_base)

        cl_pert = [10.0 + delta]
        net_pert = make_psh_net(cl=cl_pert)
        _, sol_pert = solve_psh_opf(net_pert)

        dpsh_dcl = calc_sensitivity(prob_base, :psh, :cl)
        check_psh(dpsh_dcl, sol_base, sol_pert, 1)
    end

    @testset "w.r.t. :fmax" begin
        # Use tight flow limit to create congestion-driven shedding
        net_base = make_psh_net(fmax=[0.3])
        prob_base, sol_base = solve_psh_opf(net_base)

        fmax_pert = [0.3 + delta]
        net_pert = make_psh_net(fmax=fmax_pert)
        _, sol_pert = solve_psh_opf(net_pert)

        dpsh_dfmax = calc_sensitivity(prob_base, :psh, :fmax)
        check_psh(dpsh_dfmax, sol_base, sol_pert, 1)
    end
    ...
    @testset "w.r.t. :b" begin
        ...
        b_pert = [b_base[1] + delta]
        net_pert = make_psh_net(b=b_pert)
        _, sol_pert = solve_psh_opf(net_pert)

        dpsh_db = calc_sensitivity(prob_base, :psh, :b)
        check_psh(dpsh_db, sol_base, sol_pert, 1)
    end
end
